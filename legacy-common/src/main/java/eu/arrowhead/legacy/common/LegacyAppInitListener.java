...
public abstract class LegacyAppInitListener {

	//=================================================================================================
	// members

	protected final Logger logger = LogManager.getLogger(LegacyAppInitListener.class);
	...
	@Autowired
	protected LegacySystemRegistrationProperties systemRegistrationProperties;
	...
	public void onApplicationEvent(final ContextRefreshedEvent event) throws KeyStoreException, NoSuchAlgorithmException, CertificateException, IOException, InterruptedException {
		...
		logger.info("System name: {}", getSystemName());
		...
		checkServiceRegistryConnection();
		customInit(event);
	...
	//-------------------------------------------------------------------------------------------------
	private void checkServiceRegistryConnection() throws InterruptedException {
		...
		final int retries = MAX_NUMBER_OF_SERVICE_REGISTRY_CONNECTION_RETRIES;
		final int period = WAITING_PERIOD_BETWEEN_RETRIES_IN_SECONDS;
	...
	private UriComponents createEchoUri(final String scheme) {
		...
		return Utilities.createURI(scheme, systemRegistrationProperties.getServiceRegistryAddress(), systemRegistrationProperties.getServiceRegistryPort(), echoUriStr);
	}
}